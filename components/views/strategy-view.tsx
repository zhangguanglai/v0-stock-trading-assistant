...
import type { TradingStrategy, TradingCycle } from '@/lib/types';
...
const buySignalOptions = [
  { key: 'ma5CrossMa20', label: '均线多头排列', desc: 'MA5 > MA10 > MA20' },
  { key: 'macdGoldenCross', label: '日MACD金叉且零轴上方', desc: 'DIF上穿DEA且DIF>0' },
  { key: 'candleConfirm', label: 'K线确认', desc: '阳线且收盘价站上MA5' },
  { key: 'volumeConfirm', label: '成交量确认', desc: '当日量 > 20日均量×1.2' },
] as const;
...
export function StrategyView() {
  ...
  const toggleBuySignal = (strategyId: string, signalKey: string) => {
    const strategy = strategies.find((s) => s.id === strategyId);
    if (!strategy) return;

    updateStrategy(strategyId, {
      buyRules: { ...strategy.buyRules, [signalKey]: !strategy.buyRules[signalKey as keyof typeof strategy.buyRules] },
    });
  };
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <Label>最小流通市值</Label>
                        <span className="text-sm text-muted-foreground">
                          ≥ {activeStrategy.stockRules.minMarketCap}亿
                        </span>
                      </div>
                      <Slider
                        value={[activeStrategy.stockRules.minMarketCap || 30]}
                        min={0}
                        max={100}
                        step={5}
                        onValueChange={([v]) =>
                          handleUpdateRules(activeStrategy.id, 'stockRules.minMarketCap', v)
                        }
                      />
                    </div>
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <Label>最大流通市值</Label>
                        <span className="text-sm text-muted-foreground">
                          {activeStrategy.stockRules.maxMarketCap === 0 ? '不限' : `≤ ${activeStrategy.stockRules.maxMarketCap}亿`}
                        </span>
                        <Switch
                          checked={activeStrategy.stockRules.maxMarketCap === 0}
                          onCheckedChange={(checked) =>
                            handleUpdateRules(activeStrategy.id, 'stockRules.maxMarketCap', checked ? 0 : 200)
                          }
                        />
                      </div>
                      <Slider
                        value={[activeStrategy.stockRules.maxMarketCap === 0 ? 200 : activeStrategy.stockRules.maxMarketCap]}
                        min={50}
                        max={1000}
                        step={10}
                        disabled={activeStrategy.stockRules.maxMarketCap === 0}
                        onValueChange={([v]) =>
              ...
              <TabsContent value="buy" className="space-y-4">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>触发买入的技术条件（满足所有勾选条件时触发信号）</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {buySignalOptions.map((option) => (
                        <div
                          key={option.key}
                          className={`flex items-center justify-between rounded-lg border p-4 transition-all ${
                            activeStrategy.buyRules[option.key as keyof typeof activeStrategy.buyRules]
                        ...
                        >
                          ...
                          <Switch
                            checked={activeStrategy.buyRules[option.key as keyof typeof activeStrategy.buyRules] as boolean}
                            onCheckedChange={() => toggleBuySignal(activeStrategy.id, option.key)}
                          />
                        </div>
                      ))}
                    </div>
